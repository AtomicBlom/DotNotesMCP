...
.DESCRIPTION
    ...
    different reasons. deploy.ps1 promotes a build over the running copy so the next piece of work
    can be done against it; an installer is a release artifact, built on a tag. Folding it in would
    make every deploy depend on a toolchain that has nothing to do with the everyday loop.
...
.EXAMPLE
    ...
    The usual pair: stage both architectures, then compile the installer from what was staged.
...
if (-not $Stage) { $Stage = "$repository/artifacts/stage/win" }
if (-not $OutputDir) { $OutputDir = "$repository/artifacts" }
...
function Resolve-Iscc
{
    ...
    # an unelevated install is what a developer machine gets by default.
    $candidates = @(
...
function Assert-Stage
{
    ...
    # files silently installs nothing for that architecture -- which presents as an install that
    # completed and a server that is not there.
    foreach ($rid in 'win-x64', 'win-arm64')
    {
        if (-not (Test-Path "$Stage/payload/$rid/DotNotes.Server.exe"))
        {
    ...
    foreach ($script in 'DotNotes.Deploy.ps1', 'install.ps1')
    {
        if (-not (Test-Path "$Stage/$script"))
        {
            throw "the stage at $Stage has no $script. The installer runs it to stop a running " +
...
{
    <#
        That the script the installer embeds is the one in tools/.

        Everything else in a stage comes out of a publish, so it is as new as the build that made it.
        This one is copied from the repository instead, which means a stage built before an edit
        ...
        reports success, and quietly does not stop the server it was supposed to stop, because the
        copy it ran is the copy that could not find it.

        Refusing rather than silently refreshing, because a stage stale in this is usually a stage
        that predates other changes too, and only packaging again can say.
    #>
    param([Parameter(Mandatory)][string] $Stage)

    $script = 'DotNotes.Deploy.ps1'
...
if (-not $Version) { $Version = Get-PackageVersion -PayloadRoot "$Stage/payload/win-x64" }
...
$arguments = @(
    ...
    "/DAppVersion=$Version",
    "/O$($OutputDir.Replace('/', '\'))",
    "$repository/installer/dotnotes.iss".Replace('/', '\')
)

& $compiler @arguments
if ($LASTEXITCODE -ne 0) { throw "ISCC failed (exited $LASTEXITCODE)" }

$installer = "$OutputDir/dotnotes-setup.exe"
